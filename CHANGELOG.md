...
- Which account is which: each card shows the email Claude Code uses for that account (on its second line, or on the name's tooltip when the card has a note), the search finds it, the sidebar's tooltip names it, and two accounts on the same Claude account are pointed out. Brainmerge reads only the three display fields Claude Code records in its `.claude.json` (email, display name, organization name), never a password or a token, and stores the email nowhere: not in its state, a memory, a commit, Claude's instructions or the command line's output.
- The edit sheet says which email Claude Code uses, offers Claude Code's display name as the name when it is free, and, when the names ended up on each other's account, swaps them in one click (both accounts renamed at once; an open secondary account is said and nothing changes).
- Reading project paths from `.claude.json` now loads only the keys of `projects`, never their settings.
- Your first account can be edited while Claude runs: its name, color, photo and note are saved without quitting Claude, since nothing of Claude is rebuilt (only a new memory still waits for Claude to quit, because moving its links is not atomic). The edit sheet says so: "Changes apply when you save. Claude stays open."
...
- Apps you made yourself are recognized: a copy of Claude whose launch script opens one of your accounts (like a hand-made "Claude Second") is listed in that account's edit sheet, with where it is and a "Show in Finder" button. When it runs an older Claude than the one installed, the sheet and `brainmerge doctor` say that it can damage the account's data and that it looks exactly like Claude in the Dock, and suggest using Brainmerge's app and moving the copy to the Trash yourself once the account is closed. Brainmerge only reads these apps (their `Info.plist` and a short launch script, never run) and never opens, changes or trashes them.
