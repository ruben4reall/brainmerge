...
2. It never looks for, stores or transmits a password, a credential or a token. There is no code path for it. The one list that can carry one past it is `ps`'s: every running program's arguments pass through Brainmerge once, and it keeps only whether a line is Claude (for a Claude window, its program and its data folder), never the line. Even "Connected" is decided from file names alone, and the account a card shows is the email and name Claude Code writes down for display, not a credential.
