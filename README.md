...
- **Alive.** When Claude updates itself, the accounts that keep a copy of it (the ones with their own Dock icon) are rebuilt for the new version, on their own or with one click. Claude's version is checked every few minutes and whenever the window comes to the front. A window opened before the update says so on its card and in the sidebar, with a Restart button (or Restart When Idle, once its Claude Code sessions end). If Claude's own restart brings it back on the first account's folders, Brainmerge says so and offers to reopen the right account. Nothing is quit without a click.
...
3. Add a second account whenever you like. Its card shows "Log in": Brainmerge closes your other Claude windows (the login link from your browser opens in whichever one runs), opens the account for you to log in as usual, then reopens the others when you click. Cancel reopens them too. It never touches your browser or your password. When a session expires, "Log in…" in the card's menu does the same.
