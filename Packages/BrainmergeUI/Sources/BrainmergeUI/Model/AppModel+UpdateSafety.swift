...
extension AppModel {
    ...
    /// A clean quit of an account's window, the SIGTERM Quit sends (`quitAccount`: a fake in tests, which never signal).
    func quitWindow(_ identity: Identity) {
        try? quitAccount(manager, identity)
    }

    /// Opens an account through its own app, marked as opening (`launchAccount`: a fake in tests, which never start a
    /// process).
    func launchWindow(_ slug: String) {
        do { try launchAccount(manager, slug); markOpening(slug) } catch { present(error) }
    }
