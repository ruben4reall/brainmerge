...
@Suite struct SavesWaitForYourGitTests {
    ...
    @Test func aMergeLeftOpenIsSeenFromAFolderWrittenWithoutASlash() throws {
        ...
        // Brain standardizes its root: newer Foundation keeps the missing slash (the case under test), older Foundation,
        // as on CI, adds it back from the disk. Either way the unfinished merge must be seen.
